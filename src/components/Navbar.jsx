...
const Navbar = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-20 px-4 md:px-10 py-4 md:py-6 transition-all duration-300 ${scrolled ? "md:py-4" : ""}`}
    >
      <div
        className={`max-w-7xl mx-auto flex items-center justify-between glass-nav rounded-xl px-6 md:px-8 py-4 transition-all duration-300 ${scrolled ? "shadow-lg border-white/40" : "shadow-sm"}`}
      >
        <div
              ...
              >
                <nav className="flex flex-col gap-1">
                  ...
                  <Link
                    to="/careers"
                    onClick={() => setIsOpen(false)}
                    className="text-base font-bold text-heading/80 hover:text-primary hover:bg-primary/5 px-4 py-3 rounded-xl transition-all flex items-center gap-2"
                  >
                    Careers
                    <span className="text-[9px] font-black uppercase tracking-widest text-accent-fire bg-accent-fire/10 px-2 py-0.5 rounded-full">
                      Hiring
                    </span>
                  </Link>
